...
export function HomeScreen({ navigation }: ScreenProps<'Home'>) {
  ...
  return (
    ...
    >
      <Text style={s.label}>READY TO WRAP</Text>
      <Text style={s.title}>Leave with less doubt.</Text>
      ...
      {query.data?.active.map((session) => (
        <View key={session.id} style={s.card}>
          <Text style={s.label}>CONTINUE SHOOT</Text>
          <Text style={s.heading}>{session.kitName}</Text>
          <Text style={s.text}>
            {session.checked} / {session.total} checked
          </Text>
          <Button
            title="Resume return check"
      ...
      {query.data?.kits.map((kit) => (
        <View key={kit.id} style={s.card}>
          <Text style={s.heading}>{kit.name}</Text>
          <Text style={s.muted}>{kit.itemCount} physical items</Text>
          <Button
            secondary
            title="Open kit"
      ...
      <View style={s.card}>
        <Text style={s.heading}>Create a kit</Text>
        <Field
          label="Kit name"
        ...
        <Button title="Create kit" onPress={create} disabled={!name.trim()} />
      </View>
