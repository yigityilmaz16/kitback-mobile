...
function open(path = ':memory:') {
  ...
  const db: Database = {
    ...
    runSync: (sql, ...p: Bind[]) => {
      const r = native.prepare(sql).run(...p);
      return { changes: Number(r.changes), lastInsertRowId: Number(r.lastInsertRowid) };
    },
    getFirstSync: <T>(sql: string, ...p: Bind[]) =>
      (native.prepare(sql).get(...p) as T | undefined) ?? null,
...
test('acceptance: 4/5, duplicate, unknown, missing photo, close/reopen, 5/5, finish', () => {
  ...
  try {
    ...
    assert.equal(missing[0].name, 'Gear 5');
    assert.equal(missing[0].photo, 'photo-5.jpg');
    assert.throws(() => repo.finishSession(session), /Check all/);
    native.close();
    ({ repo, native } = open(join(dir, 'kitback.db')));
    assert.equal(repo.session(session).checked, 4);
    assert.equal(repo.session(session).status, 'active');
    assert.equal(repo.scan(session, 'KB-5').kind, 'checked');
    repo.finishSession(session);
    native.close();
    ({ repo, native } = open(join(dir, 'kitback.db')));
    assert.equal(repo.session(session).status, 'completed');
    assert.equal(repo.session(session).checked, 5);
    assert.equal(repo.equipment(kit).length, 5);
    assert.equal(repo.scan(session, 'KB-1').kind, 'closed');
    native.close();
  } finally {
...
test('snapshot survives item edit, deletion and additions; photo and old code retained', () => {
  const { repo, native } = open();
  const kit = repo.addKit('Kit');
  const item = repo.saveEquipment({
    kitId: kit,
    name: 'Original',
    barcode: 'OLD',
    photo: 'original.jpg',
  ...
  assert.throws(
    () => repo.saveEquipment({ kitId: kit, name: 'Reused', barcode: 'NEW', photo: null }),
    /deleted physical/,
  );
  native.close();
});
test('one active session, unique physical identifiers, empty kits and names', () => {
  const { repo, native } = open();
  const a = repo.addKit('A');
  const b = repo.addKit('B');
  assert.throws(() => repo.startSession(a), /Add equipment/);
  assert.throws(() => repo.addKit(' '));
  repo.saveEquipment({ kitId: a, name: 'Battery #1', barcode: '001', photo: null });
  assert.throws(
    () => repo.saveEquipment({ kitId: b, name: 'Battery #2', barcode: '001', photo: null }),
    /already assigned/,
  );
  const session = repo.startSession(a);
  assert.equal(repo.startSession(a), session);
  for (let i = 0; i < 100; i++) repo.scan(session, '001');
  assert.equal(repo.session(session).checked, 1);
  repo.uncheck(session, repo.snapshot(session)[0].id);
  assert.equal(repo.session(session).checked, 0);
  repo.scan(session, '001');
  repo.finishSession(session);
  assert.throws(() => repo.uncheck(session, repo.snapshot(session)[0].id), /Completed/);
  assert.notEqual(repo.startSession(a), session);
  native.close();
});
test('migration is repeatable and rejects a newer database without reset', () => {
  const { repo, native, db } = open();
  repo.addKit('Preserve');
  repo.initialize();
  assert.equal(repo.kits().length, 1);
  db.execSync('PRAGMA user_version=99');
  assert.throws(() => repo.initialize(), /newer/);
  assert.equal(repo.kits()[0].name, 'Preserve');
  native.close();
});
...
test('old snapshot identifiers cannot be reassigned after editing an item', () => {
  const { repo, native } = open();
  const kit = repo.addKit('Kit');
  const id = repo.saveEquipment({ kitId: kit, name: 'Battery', barcode: 'FIRST', photo: null });
  repo.startSession(kit);
  repo.saveEquipment({ id, kitId: kit, name: 'Battery', barcode: 'SECOND', photo: null });
  assert.throws(
    () => repo.saveEquipment({ kitId: kit, name: 'Other battery', barcode: 'FIRST', photo: null }),
    /reserved by a previous shoot/,
  );
  native.close();
});
test('snapshot failure rolls back the entire session creation', () => {
  const { repo, native, db } = open();
  const kit = repo.addKit('Kit');
  repo.saveEquipment({ kitId: kit, name: 'Battery', barcode: 'B', photo: null });
  db.execSync(
    "CREATE TRIGGER reject_snapshot BEFORE INSERT ON session_equipment BEGIN SELECT RAISE(ABORT,'simulated disk failure'); END;",
  );
  assert.throws(() => repo.startSession(kit), /simulated disk failure/);
  assert.equal(repo.sessions().length, 0);
  db.execSync('DROP TRIGGER reject_snapshot');
  assert.equal(repo.session(repo.startSession(kit)).total, 1);
  native.close();
});
test('identifiers preserve leading zeroes, case and SQL-like payloads', () => {
  const { repo, native } = open();
  const kit = repo.addKit('Kit');
  for (const code of ['001', '1', 'abc', 'ABC', "x'); DROP TABLE kits;--"]) {
    repo.saveEquipment({ kitId: kit, name: code, barcode: code, photo: null });
  }
  const session = repo.startSession(kit);
  assert.equal(repo.scan(session, '001').kind, 'checked');
  assert.equal(repo.session(session).checked, 1);
  assert.equal(repo.scan(session, "x'); DROP TABLE kits;--").kind, 'checked');
  assert.equal(repo.kits().length, 1);
  assert.equal(repo.scan(session, 'A\nB').kind, 'invalid');
  assert.equal(repo.scan(session, 'x'.repeat(513)).kind, 'invalid');
  native.close();
});
